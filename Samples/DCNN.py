    ...
    def per_dim_conv_k_max_pooling_layer(self, x, w, b, k):
        ...
        with tf.name_scope("per_dim_conv_k_max_pooling"):
            for i in range(self.embed_size):
                ...
                # k_top pool the last dimension, so reshape the input
                # make sentence_length the last
                # k_top 操作将最后一维进行池化，因此需要reshape
    ...
    def per_dim_conv_layer(self, x, w, b):
        ...
        with tf.name_scope("per_dim_conv"):
            for i in range(len(input_unstack)):
                conv = tf.nn.relu(tf.nn.conv1d(input_unstack[i], w_unstack[i], stride=1, padding="SAME") + b_unstack[
                    i])  # [batch_size, k1+ws2-1, num_filters[1]]
                convs.append(conv)
            conv = tf.stack(convs, axis=2)
