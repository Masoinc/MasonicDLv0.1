...
teX_s = seq[data_size - 1 - test_size]
teX = seq[data_size - 1 - test_size:]
teY = pre[data_size - 1 - test_size:]
...
with tf.Session() as sess:
    ...
    for step in range(train_step):
        ...
        if step % 100 == 0:
            ...
            # test_batch = teX_s[0:10]
            # print(sess.run(y_, feed_dict={X: [test_batch]}))
            # for i in range(test_size):
            #     test_batch = teX_s[i:i + 10]
            #     # test_batch = np.expand_dims(test_batch, axis=0)
            #     predict = sess.run(y_, feed_dict={X: [test_batch]})
            #     teX_s.append(predict)
            # print(teX_s)
            # print(teY)
