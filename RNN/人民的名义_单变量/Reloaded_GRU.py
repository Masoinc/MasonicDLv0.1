...
plt_title = "GRU predictions for 《人民的名义》"
...
with tf.name_scope(name='NeuralNetwork'):
    def rnn(X, W):
        ...
        # fc1 = tf.nn.dropout(fc1, keep_prob=0.95)
        fc2 = tf.nn.tanh(tf.matmul(fc1, w2) + b2)
        # fc2 = tf.nn.dropout(fc2, keep_prob=0.95)
        fc3 = tf.nn.tanh(tf.matmul(fc2, w3) + b3)
        # fc3 = tf.nn.dropout(fc3, keep_prob=0.95)
        y_ = tf.squeeze(fc3)
        return y_, W_
    ...
    for step in range(train_step):
        ...
        if step % 100 == 0 and prev_loss != -1:
            ...
            if delta < early_stopping_rate:
                prev_loss = -1
                break
            if pow(train_loss, 0.5) + pow(test_loss, 0.5) < 0.1:
                nice = True
                print("Nice result for train={0}, test={1}".format(pow(train_loss, 0.5), pow(test_loss, 0.5)))
