...
with tf.name_scope(name='NeuralNetwork'):
    def nn(X, W, seq_size, vector_size):
        ...
        for i in range(vector_size):
            Vec_Slice = tf.unstack(X, axis=2)[i]
            # [18, 15]
            conv_per_batch = []
            for k in range(batch_size):
                Batch_Slice = Vec_Slice[k]
                Batch_Slice = tf.expand_dims(Batch_Slice, 0)
                Batch_Slice = tf.expand_dims(Batch_Slice, 2)
                ...
                conv_per_batch.append(conv1)
            conv_per_batch = tf.concat(conv_per_batch, 0)
            conv.append(conv_per_batch)
...
with tf.Session() as sess:
    ...
    for i in range(step):
        ...
        for end in range(batch_size, len(trY), batch_size):
            ...
            summary,train_loss, _ = sess.run([merged,loss, train_op], feed_dict={X: x, Y: y})
            if k == 0:
        ...
        if i % 100 == 0 and i > 1:
            ...
            X.set_shape([np.shape(trX_sliced)[0], seq_size, vector_size])
            ...
            if i == step - 1:
                ...
                sample = pd.read_csv("G:\Idea\MasonicDLv0.1\Database\Sample_JiChengRen.csv", header=None, skip_blank_lines=True)
                ...
                for k in range(len(seq1) - seq_size):
                    ...
                    # sampleX[18, 15, 2]
                    sampleY.append(seq1[k + seq_size])
                    # sampleY[18, 1]
